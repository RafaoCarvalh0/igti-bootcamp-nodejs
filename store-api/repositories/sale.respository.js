...
async function insertSale(sale) {
    ...
    try {
        ...
        const values = [sale.value, new Date(), sale.client_id, sale.product_id];
...
async function updateSale(sale) {
    const conn = await connect();
    try {
        const sql = "UPDATE sales SET client_id = $1, product_id = $2, value = $3 WHERE sale_id = $4 RETURNING *"
        const values = [sale.client_id, sale.product_id, sale.value, sale.sale_id];
        const res = await conn.query(sql, values);
        return res.rows[0]
    } catch (err) {
        throw err;
